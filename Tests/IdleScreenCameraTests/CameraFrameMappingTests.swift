...
struct CameraFrameMappingTests {
  ...
  func rejectedDescriptorSkipsPayloadCopy() throws {
    ...
    guard case .descriptor(let observed) = read else {
  ...
  func duplicateHeaderFastPathMeasurement() throws {
    ...
    for _ in 0..<iterations {
      let read: CameraFrameSourceMappingRead<Void>? =
        try mapping
        .withStableSnapshot(
          copyingPixelsWhen: { _ in false },
    ...
    let hostBrand = benchmarkHostBrand()
    let enforcesNamedHostBudget = hostBrand == "Apple M4 Pro"
    print(
  ...
  /// Reports the CPU brand so the duplicate-copy budget only binds the host it
  /// was measured on. A shared runner shares cores with other jobs, so the
  /// ratio between two wall-clock loops there measures the neighbours, not the
  /// fast path.
  private func benchmarkHostBrand() -> String {
    #if canImport(Darwin)
      var size = 0
      ...
      let terminated = brand.prefix { $0 != 0 }
  ...
  func fileTypeAndPermissionsRejected() throws {
    ...
    #expect(
      throws:
        IdleScreenCameraFrameMappingError
