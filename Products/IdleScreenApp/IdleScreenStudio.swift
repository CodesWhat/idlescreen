...
struct StudioView: View {
        ...
        ZStack {
            ...
            VStack(spacing: 10) {
                HStack(spacing: 7) {
                    ...
                    Button { model.startScreenSaver() } label: {
    ...
    private var compactSettingsDock: some View {
        HStack(spacing: 13) {
            ...
            LabeledSlider(title: "Size", value: Binding(
                get: { model.configuration.appearance.glyphScale },
                set: { model.updateGlyphScale($0) }
            ))
            LabeledSlider(title: "Contrast", value: Binding(
        ...
        Menu {
            Section("Camera") {
                ...
                ForEach(connectedCameraDevices) { device in
                    Button {
                        guard let selection = IdleScreenCameraSelection.deviceIfValid(
            ...
            HStack(spacing: 6) {
                Image(systemName: model.configuration.camera.preferredDeviceIdentifier == nil
                ...
                ExpandedStudioSection(
                    ...
                    if expandedScaleControlApplies {
                        ExpandedStudioSlider(
                            ...
                            range: IdleScreenCreativeSettings.minimumScale...IdleScreenCreativeSettings.maximumScale,
    ...
    private var connectedCameraDevices: [IdleScreenCompanionCameraDevice] {
        guard case let .live(snapshot) = cameraClient.cameraDeviceState else {
            return []
        }
    ...
    var body: some View {
        HStack(spacing: 8) {
            RoundedRectangle(cornerRadius: 6)
                ...
                .overlay {
                    Image(systemName: savedLook.snapshot.source == .camera
                        ? "camera.fill"
...
private struct IdleScreenMetalViewHost: NSViewRepresentable {
    ...
    final class Coordinator {
        ...
        var cameraFrameSubscription:
            IdleScreenRendererCameraFrameRelay.Subscription?
...
private final class IdleScreenTrackedMetalView: MTKView {
    ...
    private func publishPointerLocation(from event: NSEvent) {
        ...
        pointerLocationDidChange?(CGPoint(
...
private struct IdleScreenLoupeView: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            VStack {
                Spacer()
                Text(Double(Self.magnification).formatted(
                    .number.precision(.fractionLength(2))
                ) + "×")
...
private struct IdleScreenLoupeCanvas: View {
    ...
    private func drawLoupe(
        ...
        let usesCameraColor = configuration.palette.caseInsensitiveCompare(
            "Camera Color"
        ) == .orderedSame
        ...
        for row in firstRow...lastRow {
            for column in firstColumn...lastColumn {
                ...
                let foreground = if usesCameraColor,
                                    mode != .generative,
                                    let cameraSample {
                    cameraSample.color
                } else {
    ...
    private func sampledCameraPixel(
        ...
        guard let cameraFrame,
              ...
              cameraFrame.interleavedRGB.count
                == cameraFrame.luminance.count * 3 else {
            return nil
        }
        ...
        let cameraColumn = if configuration.cameraIsMirrored,
                              mode != .generative {
    ...
    private func resolvedCell(
        ...
        case let (.camera, .some(cameraBrightness)):
