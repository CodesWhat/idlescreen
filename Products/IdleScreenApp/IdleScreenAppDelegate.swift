...
final class IdleScreenAppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if case let .cameraAgentRebind(resultPath, previousProcessIdentifier) =
            backgroundProbe {
        ...
        guard IdleScreenLaunchPolicy.shouldShowMainWindow(
            arguments: arguments
        ) else {
            logger.info("Background lifecycle probe will not activate or reveal the main window")
            if case let .configurationContrast(contrast) = backgroundProbe {
    ...
    func applicationDidChangeScreenParameters(_ notification: Notification) {
        guard let mainWindow,
              ...
              !mainWindow.styleMask.contains(.fullScreen) else {
            return
        }
    ...
    private func updateMainWindowPresentationState() {
        ...
        let isPresented = NSApp.isActive
            && !NSApp.isHidden
    ...
    private func applyScreenAspectRatio(to window: NSWindow) {
        ...
        let currentContentSize = window.contentRect(forFrameRect: window.frame)
            .size
        var targetContentSize = IdleScreenPreviewWindowGeometry
        ...
        if scale < 1,
           ...
           targetContentSize.height * scale >= minimumContentSize.height {
            targetContentSize = IdleScreenPreviewWindowGeometry
                .normalizedContentSize(
                    current: CGSize(
    ...
    private static func makeCameraClient(
        ...
        let serviceIdentity = (
            infoDictionary[CameraClientBootstrap.appGroupInfoKey] as? String
        ).flatMap { appGroupIdentifier in
        ...
        return IdleScreenCompanionCameraClient(
            ...
            effects: IdleScreenCompanionCameraEffects(
                ...
                assessAgentIdentity: { identity, remoteProcessIdentifier in
                    ...
                    guard let fingerprint = try? CameraAgentArtifactFingerprint(
    ...
    private static func cameraReplacement(
        ...
        case let .failed(_, state, diagnostic):
            return .failed(
